...
def bfs_search(initial_state):
    ...
    frontier_ = set()
    frontier_.add(initial_state.config)
    explored = set()
    max_memory = 0
    while len(frontier):
...
def dfs_search(initial_state):
    ...
    while len(frontier):
        ...
        for neighbor in neighbors:
            ...
            if delta_mem > max_memory:
                max_memory = delta_mem
            if neighbor.config not in explored and neighbor.config not in frontier_:
                frontier.append(neighbor)
                frontier_.add(neighbor.config)
                if neighbor.cost > MaxSearchDeep:
                    MaxSearchDeep = neighbor.cost
    end_time = time.time()
    search_depth = state.cost
...
if __name__ == '__main__':
    ...
    #hard_state1=PuzzleState([6, 1, 8, 4, 0, 2, 7, 3, 5],3)
    #bfs_search(hard_state)
    #dfs_search(hard_state)
    #A_star_search(hard_state)
    #bfs_search(hard_state1)
    #dfs_search(hard_state1)
    #A_star_search(hard_state)
